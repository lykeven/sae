...
void * Word2Vec::TrainThread(void* args){
    ...
    Word2Vec *self = (Word2Vec *)args;
    for(int i=0; i<self->M; i++)
    {
        for(int j=0; j<self->corpus[i].size(); j++)
        {
            ...
            {
                if(p<0||p>=corpus.size()||p==j) continue;
                for(int d=0; d<self->D; d++)
                    C[d] += self->Vec[corpus[p]][d];
            }
...
std::vector<std::vector<double> > Word2Vec::solve(vector<vector<vid_t>>corpus, int D, int W, int K, int I, double alpha) {
    ...
    //initialize node embedding vector between
    ...
    for (int i=0; i<I; i++)
        pthread_create(&pt[i], NULL, Word2Vec::TrainThread, (void *)this);
	for (int i=0; i<I; i++)
